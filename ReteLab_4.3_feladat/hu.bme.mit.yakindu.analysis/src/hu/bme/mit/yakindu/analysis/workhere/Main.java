		...
		while (iterator.hasNext()) {
			...
			if(content instanceof State) {
				...
				for (Transition t : state.getIncomingTransitions()) {
					State in = (State) t.getTarget();
					System.out.println("bejovo: " + state.getName() + " -> " + in.getName());
				}
				
				
				if (state.getOutgoingTransitions().size() == 0) {
					System.out.println("Nincs el: " + state.getName());
				}
				
				if(state.getName() == "") {
					state.setName("Ujnev" + nev);
					nev++;
					System.out.println("Uj neve: " + state.getName());
				}
